...
template <typename T>
T* PushIntoListFromMemoryPool(LinkedList<T>* list, MemoryPool* pool) {
  LinkNode<T>* node = PushIntoMemoryPool<LinkNode<T>>(pool);
  PushIntoList(list, node);
  return &node->value;
}

template <typename T>
T* PushIntoListFromMemoryPool(LinkedList<T>* list, MemoryPool* pool, T t) {
  LinkNode<T>* node = PushIntoMemoryPool<LinkNode<T>>(pool);
  PushIntoList(list, node);
